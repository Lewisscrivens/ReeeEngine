...
namespace ReeeEngine
{
	...
	{
		// Fill messages map with each possible messages string conversion.
		messageMap = {
	...
	std::string WindowsMessageManager::operator()(DWORD msg, LPARAM lp, WPARAM wp) const noexcept
	{
		constexpr int maxCharPerLine = 25; // Max number of characters per line.
		const auto message = messageMap.find(msg); // Get string for current msg DWORD-->string.
		std::ostringstream messageStream;

		// If there is an error message format it into lines.
		if (message != messageMap.end())
		{
			messageStream << std::left << std::setw(maxCharPerLine) << message->second << std::right;
		}
		// Otherwise format an unknown message with the given error code of the msg and format it onto the current message string.
		else
		{
			std::ostringstream unknownMessageStream;
			unknownMessageStream << "Unknown message: 0x" << std::hex << msg;
			messageStream << std::left << std::setw(maxCharPerLine) << unknownMessageStream.str() << std::right;
		}

		// Construct LP and WP error codes in the string.
		messageStream << "   LP: 0x" << std::hex << std::setfill('0') << std::setw(8) << lp;
		messageStream << "   WP: 0x" << std::hex << std::setfill('0') << std::setw(8) << wp << std::endl;
